...
export const adminApiHandlers = {
  ...
  [ACTIONS.RESET_COUNTER_SANCTIONS_DB_WITH_FILE]: (
    payload: Request<{ buffer: Buffer }>,
  ) => adminService.processCounterSanctionsFile(payload.buffer),
  ...
  [ACTIONS.LOAD_USERS_TARIFFS]: () => adminService.getUserTariffs(),
  [ACTIONS.GRANT_TARIFF]: (payload: Request<GrantUserTarrifPayload>) =>
  ...
  [ACTIONS.SAVE_COUNTER_SANCTIONS_TARRIFS_SETTINGS]: (
    payload: Request<CounterSanctionsTarrifSettings>,
  ) => adminService.changeCounterSanctionsTarrifSettings(payload),
  [ACTIONS.LOAD_COUNTER_SANCTIONS_TARRIFS_SETTINGS]: () =>
    adminService.getCounterSanctionsTarrifSettings(),
};
